...
## AI 听歌报告演示版（2026-09-03）

- 左下角设置、刷新和下载管理旁新增 AI 图标按钮，使用用户提供的 `resources/icons/ai.png`，点击后进入主内容栈第 `8` 页“AI 听歌报告”。
- 新增 `ui/AiReportPage`：进入页面后先显示约 `10s` 的伪加载和动态省略号，再显示固定中文听歌报告与推荐歌曲；提供“返回”和“重新生成”按钮。
- 当前版本不接入外部 AI API、不读取播放历史、不新增或修改数据库表，内容是演示用固定文本；后续接真实 API 时需要替换 `AiReportPage` 的演示逻辑。
- 回归覆盖：`tst_uiwidgets` 新增 AI 按钮尺寸/间距/信号、页面加载状态、完成后的固定报告和重复重置测试；完整 Qt CTest 9/9 通过（约 49 秒）。
