...
- `--page <0-8>`: 配 `--screenshot` 切页
- `--screenshot <png>`: 约 1.8s 后 `grab()` 并退出
- `--smoke`: 1.5s 后退出

页面栈索引:`0推荐 / 1收藏 / 2本地歌单 / 3自建歌单 / 4歌单详情 / 5正在播放 / 6搜索 / 7下载管理 / 8 AI 听歌报告`。

## AI 听歌报告与流式输出（2026-09-03）

- 左下角设置、刷新和下载管理旁新增 AI 图标按钮，使用用户提供的 `resources/icons/ai.png`，点击后进入主内容栈第 `8` 页“AI 听歌报告”。
- 新增 `ui/AiReportPage`：进入页面后提示“正在根据历史听歌总结…”，首个字符前等待约 `10s`，再以流式文字逐段输出三套华语流行主题的长篇听歌报告与推荐歌曲；每次“重新生成”会轮换一套文案，提供“返回”和“重新生成”按钮。
- AI 图标改为使用界面主题红色（`ThemeColor::Accent`）渲染，主题切换时保持同步。
- 当前版本不接入外部 AI API、不读取播放历史、不新增或修改数据库表，内容是演示用固定文本；后续接真实 API 时需要替换 `AiReportPage` 的演示逻辑。
...
## 收藏与自建歌单同曲合并（2026-09-01）

- 收藏页和自建歌单详情现在复用搜索聚合器的确定性同曲规则：规范化歌名、主要歌手、专辑和版本标签一致，双方时长有效且误差不超过 3000ms 时，一个歌单内只显示一个歌曲块；第一次出现的位置决定歌单显示顺序，底层本地、网易云和 QQ 记录全部保留在歌曲块中。
- 合并只在收藏和自建歌单启用。本地歌单、普通歌手/专辑详情、播放列表和平台云歌单保持既有独立行语义，避免扩大搜索去重的产品边界。
- 单行播放、播放全部、下载和删除下载使用歌曲块当前选中的来源；收藏、取消收藏、添加到已有/新建歌单以及从当前自建歌单移除，会覆盖该歌曲块内全部来源成员。批量收藏和添加歌单同样按成员展开，批量下载及按来源删除仍只处理每块当前来源。
- `SongListModel` 同时保存每行的稳定组身份、当前来源代表和全部来源成员；来源按钮继续按“本地、网易云、QQ”优先级选择可用代表。自建歌单移除改为事务批量删除全部成员并只刷新一次，歌单标题中的歌曲数和总时长按合并后歌曲块计算。
- 新增回归覆盖保序聚合、自建歌单事务批量移除、合并行成员与批量操作、收藏/自建歌单只显示一次、普通详情不合并，以及切到 QQ 来源后播放队列使用 QQ 版本。

### 本轮编译/测试记录

- 新增合并行批量测试首次失败：点击“全选”后 `selectedSongs()` 为 0。根因是全选逻辑写入当前活动歌曲的 `selectionIdentity()`，而合并行选择状态使用稳定的 `SearchResultGroup::identity`，两种身份不一致。可行方案是所有行选择入口统一使用 `SongListModel::rowIdentityAt()`；修复后最新 `tst_songlistview` 定向测试通过。
...
## 播放器与歌曲列表视觉细化（2026-09-01）

- 最终方案记录在 `docs/PLAYERBAR_VISUAL_REFINEMENT_PLAN.md`。搜索结果关键词删除半透明红底，只保留主题红文字；播放器高度由 `224px` 调整为 `204px`，背景与页面统一为 `#0E0E14`，删除播放器自绘灰底和来源/缓存/下载文字标签。
- 播放器左侧改为圆角封面、歌名、歌手和横向排列的收藏/下载按钮，控制区整体左移；宽屏封面 `128px`、中等宽度 `104px`，小于 `780px` 时继续隐藏封面、歌词、队列和独立音量条，但保留歌名、歌手及核心播放控件。完整文字通过右侧省略和 tooltip 保留，播放错误改由播放器、标题、封面 tooltip 暴露。
- 使用用户提供的列表循环、单曲循环、随机播放、歌词和播放列表 PNG，按有效 alpha 边界居中并统一着色为 `#B8B8C4`；歌词最终采用 `C:\Users\Fusssssion\Downloads\歌词.png`。三种模式索引和所有既有信号保持不变；未导入“云.png”和“下载本地.png”，旧 `icon-cloud.svg` 已移除。
- 歌曲行收藏图标名义尺寸从 `18px` 放大到 `24px`，下载箭头和线宽约放大 `1.3` 倍，loading 圆环改为 `21px`、删除图标改为 `20px`，悬停不再产生纵向偏移。下载、完成和删除仍完全绑定真实 `DownloadService` 状态。
- 新增回归覆盖播放器背景/高度、歌名歌手、无状态标签、错误 tooltip、真实及占位封面圆角、操作按钮横向排列、新 PNG 资源和模式差异、关键词无红底、歌曲行操作图标垂直中心及高 DPI/紧凑布局。定向 `tst_songlistview` 最终 31/31 通过。

### 本轮编译/测试与避错记录

- 第一次定向 CTest 为 30 通过、1 失败，失败在新增 `songRowActionIconsStayVerticallyCentered`。初始测试错误地使用包含顶部 `42px` 和底部 `16px` 安全留白的整行 `visualRect` 作为歌曲块中心；改为按实际 `112px` 内容区计算后，数值诊断又确认收藏图标仍同时向左、向上偏移约 `4px`。真实根因是旧 `tintedIcon()` 把带 `1.5x DPR` 的 SVG pixmap 绘入默认 `1x DPR` 画布，只使用画布左上角约三分之二；修复为用 `QSvgRenderer` 按目标 DPR 直接渲染和着色。该问题属于生产 UI 的既有高 DPI 缺陷，不是下载状态机或点击区域问题。
- 为输出上述坐标诊断时，`tst_songlistview.cpp` 编译失败，MinGW 明确报 `QRect has no member named toString`。根因是误用了不存在的 Qt API，不属于环境问题；可行方案是显式输出 `x/y/width/height`，修正后编译通过。其间另一次构建审批等待超时，命令没有启动；原样重试后得到上述真实 C++ 诊断。
- 完整 Qt CTest 在最终构建上为 7/8：除 `tst_playerservice` 外其余 7 项全部通过，`tst_songlistview` 持续通过。`tst_playerservice` 在 CTest 中仍因真实音频 position 不推进失败；仓库工作目录直接重跑为 11 通过、3 失败、1 跳过，`Start-Process` 桌面上下文重跑为 12 通过、2 失败、1 跳过，且 `onlineUrlRetriesOnce` 明确报告“无可用音频输出设备”。根因仍是既有 Qt Multimedia 自动化进程没有活动输出时钟，不是本轮仅修改 UI 绘制、资源和样式的回归；本轮未修改播放器业务、放宽断言或伪造通过。
...
## 缓存清理与推荐刷新（2026-09-01）

- 实施边界记录在 `docs/CACHE_AND_RECOMMEND_REFRESH_PLAN.md`。设置页“一键清空缓存”现在分项统计并清理自动播放音频、托管在线封面、网易云/QQ 推荐响应、搜索结果/联想/热搜响应，以及没有收藏、歌单、下载、播放、歌词或当前队列关系的纯浏览在线歌曲记录；搜索历史继续保存在 QSettings 中，不随响应缓存删除。
- 永久下载及下载清单、下载歌曲封面、本地歌曲及其封面、用户歌单封面、收藏、自建歌单、最近播放、播放次数、独立 LRC、账号凭据和搜索历史均受保护。旧版位于 `cache/` 的唯一 LRC 会先迁移到独立 `lyrics/`，迁移失败时保留对应播放缓存，不会以清缓存为由丢失歌词。
- 新增 `CacheUsageBreakdown` / `CacheClearResult`，清理结果会汇总释放空间、各类文件和纯浏览记录数量，并单独列出未完全清理项。所有删除都限制在托管缓存根目录；数据库关系在事务中更新，随后执行 WAL checkpoint 并统一刷新内存曲库。
- 推荐页的歌曲与歌单请求改为并行；QQ 推荐歌单使用平台 `topPlaylists()`，不再把用户云歌单冒充推荐。手动刷新每次把歌单偏移增加 6，平台返回空页时仅回退一次到第 0 页；响应完全相同会明确提示“平台返回内容未变化”。推荐缓存升级为 version 2 的原子 `QSaveFile`，按网易云/QQ 分离保存时间和偏移。
- 侧栏底部新增 `SidebarFooter`，设置按钮与刷新按钮均为 `28×28px`、间距 `12px`、左/下边距 `18px`；刷新图标为 `18×18px`、静态 `#B8B8C4`，没有旋转或其他动效。点击后重载数据库、歌单、当前页面和全局歌曲状态，再强制刷新当前选中的网易云或 QQ 推荐，并用 tooltip 返回结果。
- 清缓存和刷新都增加请求代次及 `QPointer` 生命周期保护。搜索、推荐和主窗口封面下载的迟到成功文件会移除，不能在清理完成后把旧封面重新写回界面或数据库。

### 本轮编译/测试与避错记录

- 第一次通过 PowerShell 直接调用 CMake/Ninja 并使用 `-j2` 编译时，`LibraryService.cpp` 与 AutoMoc 生成单元同时以退出码 1 结束且没有任何 C++ 诊断；改为 `-j1` 直接重试仍在 AutoMoc 编译单元无诊断退出。根因是本机已知的 PowerShell/受控桌面直接拉起 MinGW 子进程不稳定，不是源码错误；可行方案是在 `W:\build-ascii` 放置 ASCII `.bat`，由 `cmd.exe` 设置 Qt、MinGW、vcpkg PATH 后运行 Ninja `-j1`。该方案随后编译全部目标成功。
- 新增 UI 测试首轮为 31 通过、2 失败。刷新图标测试逐个要求所有半透明抗锯齿像素都等于目标 RGB，实际边缘像素被预乘后为 `170`；QQ 推荐测试使用默认 Native QSettings，在测试执行上下文无法写入注册表，因此登录标识回读为空、网络方法没有被调用。可行方案是图标只校验最高不透明度像素，并把测试设置隔离到临时 INI；生产按钮着色同时改为显式保留源 alpha 的逐像素着色。两项均通过。
- 上述图标测试第一次修正后仍有 1 项失败：默认构造的 `QColor` alpha 为 255，使用它作为“当前最大 alpha”导致循环从未选中真实像素，最终误读 RGB 为 0。根因是测试哨兵初始化错误；改用独立 `strongestAlpha=-1` 后完整 `tst_songlistview` 通过。
- 正式目录首次切换时，旧播放器虽被定向停止但 DLL 仍短暂占用，首个 `Move-Item` 失败；PowerShell 默认将该错误视为非终止错误，随后把新包移动成了旧目录的子目录。没有文件被删除。可行方案是先只读核对目录，随后设置 `$ErrorActionPreference='Stop'`、验证所有解析后路径、把新包恢复到同级目录，再以“旧目录改名备份 -> 新目录改名正式目录”的顺序切换，并在第二步失败时自动回滚。本轮已按该方案安全完成，后续发布必须继续显式使用终止错误语义。
- 新发布包校验时曾在非提升 shell 中读取 `W:`，报“Cannot find drive W”。根因是该 shell 看不到完整桌面上下文建立的 `subst` 映射，不是构建产物缺失；改用仓库实际 `build-ascii` 绝对路径后，构建与发布 EXE 哈希一致。

### 最终验证与发布

- 最终 `W:\build-ascii` 全部目标构建成功；完整 8 项 Qt CTest 为 8/8，0 失败，总计 50.36 秒，包含播放器真实音频计时、缓存数据安全、搜索、DPAPI、多来源及响应式 UI。QQ Node 契约测试 8/8 通过。
- 新增缓存回归确认：纯浏览行会删除；收藏、歌单、最近播放、播放次数、当前队列、独立歌词、本地歌曲/封面、永久下载/封面和用户歌单封面保留；自动音频、在线封面、推荐/搜索响应删除；搜索历史保留。新增 UI 回归确认刷新按钮尺寸、间距、边距、图标 alpha/色彩、点击信号，QQ 强制刷新使用 `topPlaylists()`、偏移递增且内容未变化有明确反馈。
- 构建包使用空音乐目录、独立 SQLite、独立 QSettings 与独立下载目录完成 1280×800 smoke，退出码为 0；另完成 940×600、150% DPI 截图，目视确认侧栏底部设置/刷新按钮无裁切、间距和对齐正确。发布包再次以独立数据执行 smoke，退出码为 0，数据库和截图均生成。
- 新发布目录包含 38 个文件、90,414,380 字节；构建 EXE 与发布 EXE SHA-256 均为 `E696479AF3639274DA0625704D52699E7F7336296D6FC0913E64CE21BAF97008`。新包已切换到快捷方式固定目标 `dist\NeteaseClone\NeteaseClone.exe`；被替换版本保留在 `dist\NeteaseClone.previous-20260901-194533`，快捷方式目标和工作目录均已复核一致。

## QQ VIP、云端自建歌单与播放器交互修复（2026-09-01）

- QQ 搜索歌曲新增独立 `mediaRemoteId`，从 `file.media_mid/strMediaMid` 规范化并贯穿 `Song`、SQLite `songs.media_remote_id`、损坏库恢复、下载清单/下载重关联、收藏/歌单读取、播放器和下载器。`mediaRemoteId` 只参与实际音频地址解析，不参与同曲查重；播放器和下载器现在把完整歌曲传给来源，QQ `/v1/media` 使用 `{remoteId,mediaRemoteId}` 调用上游 `getMusicPlay({songmid,mediaId})`。
- QQ 包装层不再把所有空播放地址泛化为 VIP 错误，而是保留上游详细原因；新增 `/v1/account/vip`，账号中心显示“VIP 已识别 / 普通账号 / VIP 状态待验证”。VIP 身份不绕过版权、DRM、地区、403 或平台未下发播放地址的限制。
- 网易云和 QQ 登录验证成功后分别加载账号云歌单。云歌单元数据使用 `cloud-playlists.json` 原子缓存，启动先按已保存账号展示缓存，再异步按来源刷新；退出某一来源只移除该来源条目。侧栏自建歌单行右侧显示不可单独点击的网易云/QQ Logo，整行仍打开只读在线歌单；自建歌单页也展示云歌单。云歌单不写入本地歌单表，不允许编辑、重命名或删除，也不在登录时批量写入全部歌曲。
- 歌单详情上下文拆分为本地歌单、播放列表和只读详情。播放列表 `…` 现在提供“保存为新建歌单”和“清空播放列表”，行菜单支持“从播放列表移除”；歌手、专辑、云歌单等只读页不再显示空 `…`。点击歌曲仍以当前页面替换播放队列，没有改成追加模式。
- 搜索页临时在线歌曲不再因 `id=-1` 与默认播放 ID 相等而全部误显示为播放中。播放歌曲成为当前曲目时才写入数据库；播放标记同时使用有效数据库 ID 和稳定来源身份，因此不会为了建立队列把整页未播放浏览记录写入曲库。
- 播放器收藏/下载按钮不再在 `enterEvent` 中用已经为真的 `underMouse()` 作为动画起点，改为持久悬停状态驱动；慢速进入和快速划过都会从静止态播放完整预览动效。播放模式、歌词、播放列表和音量按钮底色改为透明，保留图标和点击功能。
- 进度条拖动期间屏蔽播放器持续位置回写，释放时在保护期内只提交一次最终 seek，并立即更新当前时间文本；播放中与暂停时现在都能一次拖放到位。歌词滚轮预览期间仍更新当前行高亮但不强制回中，最后一次滚轮操作满 5 秒才回到当前播放位置，每次新滚动都会重置计时。

### 本轮编译/测试与避错记录

- 第一次 Qt 构建在 `SettingsService.cpp` 报 `invalid use of incomplete type 'class QDir'`。根因是新增云歌单缓存路径使用 `QFileInfo::dir()`，但文件只包含 `<QFileInfo>`，`QDir` 仍是前置声明；补充 `<QDir>` 后全量 73 个后续步骤构建成功。这是源码包含缺失，不是环境配置问题。
- 受限命令第一次直接执行 `subst W: "<中文路径>"` 后，下一条命令看不到 `W:`；把中文路径写进 `cmd.exe` 又发生乱码。根因是 `subst` 映射不跨受限执行单元保留，且 `cmd` 参数链会破坏中文路径。可行方案：在同一个 PowerShell 命令中用 `(Get-Location).Path` 调用 `subst.exe`，随后运行 ASCII `W:\build-ascii\run-target-build.bat`；不要把中文路径字面量传给 `cmd`。
- 新增完整歌曲播放地址接口后，首轮 `tst_playerservice` 的两项 QQ 混合来源用例失败，QQ 假来源没有收到请求。根因是测试双只覆盖旧 `songUrls(QStringList)`，生产 `PlayerService` 已正确改用 `songUrls(QList<Song>)`；测试双补充完整歌曲重载并继续复用旧记录逻辑后，顺序、单曲循环和随机定向用例全部通过。这是测试契约未跟进，不是生产来源路由回归。
- 新增悬停测试最初依赖跨子控件 `QTest::mouseMove`，在 150% DPI 的无交互测试窗口中没有稳定产生子按钮 Enter，导致动画进度断言失败；改为向目标按钮发送明确 `QEnterEvent/QEvent::Leave` 后通过。透明按钮测试最初把按钮自身抓图的透明像素错误预期为父背景 `#0E0E14`；修正为验证 alpha 为 0 后通过。两项都是测试观察方式错误，生产绘制无需回退。
- 受限上下文首轮完整 CTest 为 4/8：`tst_multisourcesupport` 的 DPAPI 写入返回 Windows 错误 2，`tst_searchpage` 无法读回刚写入的搜索缓存，`tst_songlistview` 的推荐缓存文件无法创建；同时播放器混合来源计时在无稳定音频端点时波动。这与既有受限环境边界一致。完整桌面用户环境最终 8/8 全部通过，总计 52.33 秒；未削弱 DPAPI、缓存断言或真实音频测试。
- `windeployqt` 继续只提示可选 `dxcompiler.dll/dxil.dll` 缺失；本应用不使用对应 Direct3D 12 功能。新发布包 38 个文件、90,535,003 字节，构建 EXE 与发布 EXE SHA-256 均为 `8E403A87F43393F7C7A702A5F92B29330647538399ADC81C1A3960551C223423`。

### 最终验证与发布

- QQ Node 契约测试 10/10 通过，新增覆盖 `media_mid`、上游播放错误保留、QQ VIP 状态和微信账号大整数云歌单 ID。Qt 完整桌面回归 8/8 通过。
...
- 桌面快捷方式已显式重写并复核目标为 `dist\NeteaseClone\NeteaseClone.exe`。A/B 旧版进程一度失去窗口并持有单实例锁；正常关闭和不带 `/F` 的进程树关闭均被 Windows 拒绝，强制结束也未获执行许可，因此没有冒险中断真实数据库写入。该旧进程树随后自行退出，正式修复版已从当前发布目录成功启动，窗口响应正常，QQ 包装服务在 `127.0.0.1:3200` 监听；仍需用户在当前窗口用真人 QQ VIP 账号完成最终播放/下载确认。

## 跨机器便携包（2026-09-02）

- README 已明确区分本地离线功能与需要本地在线服务的搜索、登录和在线歌曲功能。
- 新增 `scripts/package-portable.ps1`：从 Release 构建目录生成带时间戳的便携目录和 ZIP；包含 Qt/FFmpeg/TagLib 运行库、Node.js 运行时、NeteaseCloudMusicApi@4.32.0、QQ 包装服务及 @sansenjian/qq-music-api@2.6.0、启动脚本和目标机说明。
- ApiService 和 QqApiService 增加从 EXE 所在目录发现本地部署目录和 `runtime/node/node.exe`；网易云服务启动时同步传递配置端口，便携包可以直接使用自带 Node.js。

### 本轮编译/测试与避错记录

- 便携包第一次应用自启动探测使用临时网易云端口 33100，但 ApiService 当时没有把配置端口传给 Node，导致探测未命中。根因是生产服务启动环境变量缺失，不是环境配置问题；补充 PORT 环境变量后重新构建。
- 修复后的第二次默认端口探测发现本机已有 3000 服务，健康检查提前成功，无法证明服务来自便携包。根因是测试端口被现有开发服务占用，不是便携包失败；改用未占用 33100 的隔离端口并核对应用自启动结果。
- 构建命令前执行 `subst.exe W: /D` 时在没有映射的会话输出 `Invalid parameter - W:`；根因是清理不存在映射时的命令返回方式，不影响随后重新建立 W: 并完成构建。以后忽略该清理返回值，直接在同一命令会话重新映射并运行 build-ascii 脚本。
- windeployqt 输出 dxcompiler.dll/dxil.dll 缺失警告；它们是 Direct3D 12 可选组件，本应用使用 FFmpeg 多媒体且不依赖该功能，不属于发布阻断项。
- 最终 W:\build-ascii Release 构建 12/12 成功；完整 Qt CTest 8/8 通过，用时 47.31 秒；QQ Node 契约测试 12/12 通过。
- 最终 ZIP 解压回归通过，解压后 3925 个文件，关键 EXE、Node、两个服务入口、启动脚本和清单均存在。两个内置服务在临时端口分别返回网易云 HTTP 200、QQ health HTTP 200 且服务签名正确。便携播放器使用隔离数据库和设置目录自启动包内网易云服务成功，应用正常退出。

### 便携包发布状态

- ZIP：`dist\NeteaseClone-portable-20260902-100222.zip`
- ZIP 大小：89,448,790 bytes
- ZIP SHA-256：B7F143A596585C156EB1B4C62FDEF8A437E19E6739FAE7364AC30DE58E7AF601
- 解压目录总大小：219,816,808 bytes，3925 个文件。
- 包不包含用户音乐、数据库、缓存、下载文件或账号凭据；目标机必须重新扫码登录。当前仍未完成真实新机器真人登录、在线播放和下载验收。

## 账号、游客来源、下载删除、联播与返回导航一致性（2026-09-02）

- 账号昵称来源与头像来源已经分离。QQ/微信或网易云登录成功后会记录最近活跃账号；首选账号退出后自动回退剩余账号；仅存在 QQ/微信账号时，左上角显示 QQ 昵称和头像，自定义头像不再改变昵称来源。
- 来源服务状态统一为 `Unavailable / Guest / Verifying / Authenticated`。启动保留磁盘账号资料并异步验证；验证中的来源不提前标记已认证，临时服务失败不清账号，只有明确无效资料才清理。搜索仍允许游客结果，但同曲组存在有效本地文件、缓存、永久下载或已登录版本时隐藏游客版本；全部只有游客版本时显示“游客试听”，平台服务不可用的版本不再错误标记为可播放。
- 单曲和批量下载只入队，不再自动跳转页面。侧栏设置、刷新旁新增下载状态入口；真实下载中才播放裁切箭头动画。下载页改为左右独立滚动栏，任务和完成行固定 `76px`，显示歌名、歌手、专辑、已接收/总大小、1% 进度和状态；完成任务即时进入右栏，失败/取消可在其他任务运行时重新排队。
- 永久下载和播放缓存改为先确认文件删除结果，再清数据库路径；删除失败保留按钮状态并返回具体错误。删除当前媒体前先释放 Windows 文件句柄，成功后按下载、缓存、在线来源顺序在原位置恢复；移除歌曲记录会同步播放器内存队列。缓存删除拒绝越过应用托管 `cache` 目录，LRU 淘汰也复用相同详细删除接口，不再在文件删除失败时误清数据库状态。
- 播放器为媒体加载保留播放意图和最后有效位置，同曲元数据刷新不清进度，下载/缓存/在线地址回退后恢复原位置。自然结束、接近末尾的停止状态和不可恢复错误共用带加载代次的推进保护；顺序循环、单曲循环、随机和混合来源队列统一处理，不可播放歌曲在顺序/随机模式中跳过。
- 搜索、歌单/歌手/专辑/云歌单详情、播放列表、歌词页和下载页均有可见 `68×32px` 返回按钮。返回按钮、标题栏搜索关闭和 Esc 使用同一历史；共享 `SongListPage` 保存歌曲、标题、封面、歌单菜单和来源合并上下文，详情覆盖后可恢复上一层，一级页面 Esc 不退出应用。

### 本轮编译/测试失败与避错记录

- 直接使用仓库内 `build-ascii` 时，CMake 缓存指向 `W:\build-ascii`，而新命令进程没有 `W:`，因此报告构建目录不可用；把中文仓库路径直接写入 `cmd.exe` 又发生代码页乱码。可行方案是在同一 PowerShell 执行单元用 `(Get-Location).Path` 建立 `subst W:`，随后调用 ASCII 路径下现有 `W:\build-ascii\run-target-build.bat`。
- 第一轮 MinGW AutoMoc 在受限进程中无 C++ 诊断退出；切换到上述完整桌面 ASCII `.bat` 后获得真实错误：`FileReleaseState` 的 `const& = {}` 默认实参无法转换。改为无参和带恢复状态两个重载后构建成功，不应为无诊断环境退出修改业务代码。
- 第一轮 `tst_songlistview` 为 39 通过、3 失败：高 DPI 图标用例运行在 DPR 1.0；来源二次点击用例的本地有效版本按新规则正确隐藏了游客来源；推荐歌单缓存文件在受限设置目录无法创建。解决方案分别为通过 CTest 的 `QT_SCALE_FACTOR=1.5` 属性运行、把来源交互夹具显式设为两个已认证平台、在完整桌面隔离 QSettings 目录复跑；没有放宽像素、来源或缓存断言。
- 构建前清理不存在的 `W:` 会输出 `Invalid parameter - W:`。这是 `subst /D` 对不存在映射的提示，不影响随后建立映射和 Ninja 成功；后续可直接覆盖建立当前会话映射，或忽略清理命令返回值。
- 新便携 ZIP 的首次 QQ 服务探活虽然已经返回 HTTP 200 和 `{service:"wycloudforge-qq-wrapper"}`，验证脚本却错误匹配 `wycloudforge-qq-api`，因此以退出码 3 误报失败。根因是测试脚本签名常量写错，不是包装服务或 ZIP 缺文件；改为解析 JSON 并严格比较稳定契约 `data.service == "wycloudforge-qq-wrapper"` 后，随机冷端口启动、探活和停止通过。

### 当前验证

- `W:\build-ascii` 全部目标编译链接成功；完整 Qt CTest 8/8 通过，总计 41.12 秒。新增回归覆盖游客来源隐藏、平台不可用来源、仅 QQ 账号回退、下载页 100 行固定高度和字节进度、详情导航快照、缓存越界保护、同曲元数据进度保持及播放器队列删除同步。
- QQ Node 契约测试 12/12 通过；通过的 `tst_multisourcesupport` 同时覆盖 QQ 包装服务随机冷端口启动、签名探活和停止。
- 构建包使用空音乐目录、独立 SQLite、独立 QSettings 和独立下载目录打开下载管理页，应用自行退出且退出码 0，数据库和 1280×800 截图均生成；未读取真实曲库、下载目录或账号设置。
